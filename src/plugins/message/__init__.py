...
async def get_time_table(bot: Bot, event: Event, state: T_State):
    ...
    imaegPath = await get_timetable(f"{year}-{str(month).zfill(2)}-{str(day).zfill(2)}")
    print(imaegPath,"==============================================")
    await time_table.send(MessageSegment.image(f'file:///{imaegPath}'))
...
async def get_timetable(Time=time.strftime("%Y-%m-%d", time.localtime())):
    ...
    IMG_PATH = os.path.join(BASE_PATH, 'plugins/table.jpg')
    fontPath = os.path.join(BASE_PATH, 'plugins/XinDiXiaWuChaBaiJinBan-2.ttf')
    jsText = """
    ...
    margin = 70
    padding = 50
    draw.text((padding, margin + fontSize), table[0], fill='black', font=font)
    for tab in table[1:]:
        draw.text((padding, (margin + fontSize + 60) * (table.index(tab) + 1)), tab, fill='black', font=font)
    image.save(IMG_PATH)
    return IMG_PATH
