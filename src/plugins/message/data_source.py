...
#     await help.send(message.Message('[CQ:image,file=http://a1.qpic.cn/psc?/V52atmOK2u5JUK3FVE611ZbulV4a4WS4/ruAMsa53pVQWN7FLK88i5mmwErYxDlIaFLkEt769pXwZLYCAKlC19BIaP0obEkfyi42y249jjMV6lvOi7LQTBISqOKyvsxqfu80pzhlWB04!/c&ek=1&kp=1&pt=0&bo=gAK9CwAAAAABJzQ!&tl=3&vuin=3362045101&tm=1629057600&sce=60-2-2&rf=0-0,id=40000]'))
#
# ######################################################################################
#
#
...
#         week = time.localtime(time.time()).tm_wday + 2
#     elif re.search('.*大后天.*', message):
#         week = time.localtime(time.time()).tm_wday + 3
#     else:
...
#
#
# async def get_timetable(week, Time=time.strftime("%Y-%m-%d", time.localtime())):
#     jsText = """
...
#
#
# qinnian = on_regex('.*[查看].*青年大学习.*', priority=5)
#
# @qinnian.handle()
...
#                 '李东清': '202027550209', '李家武': '202027550210', '廖瑞辉': '202027550211', '刘东洋': '202027550212',
#                 '刘潘': '202027550213', '刘伟': '202027550214', '龙庆林': '202027550215', '卢军': '202027550216',
#                 '罗剑锋': '202027550217', '彭蔚': '202027550218', '石荻松': '202027550219', '孙超': '202027550220',
...
#     msg = ''
#     bot = nonebot.get_bots()['1172314736']
#     if day_hour == 8 and day_min == 10:
#         timetable = await get_timetable(time.localtime().tm_wday)
...
# ###################################回复模式###################################
#
# mssage_send = on_message(priority=50)
#
# @mssage_send.handle()
...
#         else:
#             user_id = int(user_id)
#             groupID = STATIC_VAL.GROUP_ID
#             group_info = await get_group_number_info(groupID)
#             group_list = [userID['user_id'] for userID in group_info]
...
#             else:
#                 await batter.send(message.Message(f'抱歉 {user_id} 并非本群成员，小呆现在还只敢在本群撒野[CQ:face,id=20]'))
#
#
#
...
#         if get_user_id in group_list:
#             msg = message.Message(f'[CQ:at,qq={get_user_id}] {caihonMsg}')
#             await kuauser.send(msg)
#         else:
#             await kuauser.send(message.Message(f'抱歉 {get_user_id} 并非本群成员，小呆只能夸本群的人哦[CQ:face,id=20]'))
#     else:
...
#
#
# dinshiTable = on_regex('.*号课表.*', priority=10)
#
# @dinshiTable.handle()
